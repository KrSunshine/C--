   ...
   def update(self):
       ...
       if keys[pygame.K_s]:
          ...
          if not (self.y <= (HEIGHT -self.HEIGHT//2 - BORDER)):
           ...
           if not (self.x <= (WIDTH -self.WIDTH//2 - ENDZONE)):
              self.x = WIDTH - self.WIDTH//2 - ENDZONE
       elif keys[pygame.K_a]:
           player1.moveLeft = 1
           self.x -= (self.moveRight - self.moveLeft)+ VELOCITY
           if not (self.x >= 0 + self.WIDTH//2 + ENDZONE):
              self.x = 0 + self.WIDTH//2 + ENDZONE
...
class Player2: #shooter only moves up and down on y coordinate
   ...
   def show(self, colour):
       global screen
       pygame.draw.rect(screen, colour, pygame.Rect(WIDTH-(2*ENDZONE+WIDTH//240)-self.WIDTH, self.y-self.HEIGHT//2, self.WIDTH, self.HEIGHT))
